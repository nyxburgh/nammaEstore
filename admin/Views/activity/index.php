...
<div class="filter-bar mb-3"><form method="get" class="row g-2 align-items-end">
  ...
  <div class="col-md-2"><select name="module" class="form-select"><option value="">All Modules</option><?php foreach(['auth','sellers','customers','products','orders','banners','reports','activity','admin_roles'] as $m): ?><option value="<?= $m ?>" <?= ($filters['module']??'')===$m?'selected':'' ?>><?= ucfirst($m) ?></option><?php endforeach; ?></select></div>
  <div class="col-md-2"><select name="actor_type" class="form-select"><option value="">All Types</option><option value="admin" <?= ($filters['actor_type']??'')==='admin'?'selected':'' ?>>Super Admin</option><option value="sub_admin" <?= ($filters['actor_type']??'')==='sub_admin'?'selected':'' ?>>Sub Admin</option><option value="seller" <?= ($filters['actor_type']??'')==='seller'?'selected':'' ?>>Seller</option></select></div>
...
<div class="card"><div class="table-responsive"><table class="table mb-0">
  ...
  <?php else: $tca=['admin'=>'primary','sub_admin'=>'info','seller'=>'success','customer'=>'secondary']; foreach($logs['data'] as $log): ?>
  <tr>
