...
<div class="row g-3">
  ...
  <?= csrf_field() ?>
      ...
      <div class="card mb-3"><div class="card-header"><span class="card-title"><i class="bi bi-shop me-2"></i>Shop Details</span></div><div class="card-body"><div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <select name="vendor_type" class="form-select">
            <option value="commission" <?= ($v['vendor_type']??'commission')==='commission'?'selected':'' ?>>Commission (% per order)</option>
            <option value="subscription" <?= ($v['vendor_type']??'')==='subscription'?'selected':'' ?>>Subscription (0% commission while active)</option>
          </select>
        </div>
      ...
      <div class="card mb-3"><div class="card-header"><span class="card-title"><i class="bi bi-geo-alt me-2"></i>Registered Address <small class="text-muted">(used for GST place-of-supply)</small></span></div><div class="card-body"><div class="row g-3">
        <div class="col-md-6"><label class="form-label">Shop Phone</label><input type="text" name="shop_phone" class="form-control" value="<?= e($v['shop_phone'] ?? '') ?>"></div>
        <div class="col-md-6"><label class="form-label">Pincode</label><input type="text" name="pincode" class="form-control" value="<?= e($v['pincode'] ?? '') ?>"></div>
        <div class="col-12"><label class="form-label">Address</label><input type="text" name="address" class="form-control" value="<?= e($v['address'] ?? '') ?>"></div>
        <div class="col-md-6"><label class="form-label">City</label><input type="text" name="city" class="form-control" value="<?= e($v['city'] ?? '') ?>"></div>
        <div class="col-md-6"><label class="form-label">State</label><input type="text" name="state" class="form-control" value="<?= e($v['state'] ?? '') ?>"></div>
      </div></div></div>
      <div class="d-flex gap-2"><button type="submit" class="btn btn-primary px-4"><i class="bi bi-check-lg me-1"></i>Save Changes</button><a href="<?= $p ?>/vendors/<?= $v['id'] ?>" class="btn btn-outline-secondary">Cancel</a></div>
    </form>
  </div>
  <div class="col-lg-4"><div class="card"><div class="card-header"><span class="card-title">Current Status</span></div><div class="card-body">
    ...
    <p style="font-size:12px;color:var(--muted);margin-bottom:0;">Approve/reject/suspend actions are on the vendor's detail page — this form only edits account and shop details.</p>
  </div></div></div>
</div>
