...
class DashboardService
{
    ...
    public function getStats(): array {
        ...
        return [
            ...
            'total_customers'=>(int)$u['total_customers'],'total_vendors'=>(int)$u['total_vendors'],
            'active_vendors'=>(int)$u['active_vendors'],'pending_vendors'=>(int)$u['pending_vendors'],
    ...
    public function getTopVendors(int $limit = 5): array {
        return $this->db->fetchAll("SELECT u.name,vp.shop_name,COALESCE(SUM(ovs.gross_amount),0) as total_sales,COUNT(DISTINCT ovs.order_id) as order_count FROM `".DB_PREFIX."order_vendor_splits` ovs JOIN `".DB_PREFIX."users` u ON u.id=ovs.vendor_id LEFT JOIN `".DB_PREFIX."vendor_profiles` vp ON vp.user_id=ovs.vendor_id GROUP BY ovs.vendor_id ORDER BY total_sales DESC LIMIT ?", [$limit]);
    }
    public function getPendingVendors(int $limit = 5): array {
        return $this->db->fetchAll("SELECT u.id,u.name,u.email,u.created_at,vp.shop_name,vp.status FROM `".DB_PREFIX."users` u JOIN `".DB_PREFIX."vendor_profiles` vp ON vp.user_id=u.id WHERE vp.status='pending' ORDER BY u.created_at DESC LIMIT ?", [$limit]);
    }
    public function getCommissionSummary(): array {
        return $this->db->fetchOne("SELECT COALESCE(SUM(commission_amount),0) as total_commission,COALESCE(SUM(vendor_earning),0) as total_vendor_earning,COALESCE(AVG(commission_pct),0) as avg_commission_pct,COUNT(*) as total_transactions FROM `".DB_PREFIX."commissions` WHERE MONTH(calculated_at)=MONTH(NOW()) AND YEAR(calculated_at)=YEAR(NOW())") ?? [];
    }
}
