...
class OrderRepository extends Repository
{
    ...
    public function getOrderDetail(int $id): ?array
    {
        ...
        if (!$o) return null;
        $o['items']    = $this->db->fetchAll("SELECT oi.*, (SELECT image_path FROM `{$this->t('product_images')}` WHERE product_id=oi.product_id AND is_primary=1 LIMIT 1) as product_image, u.name as seller_name, vp.shop_name, (SELECT status FROM `{$this->t('returns')}` WHERE order_item_id=oi.id ORDER BY id DESC LIMIT 1) as return_status, (SELECT type FROM `{$this->t('returns')}` WHERE order_item_id=oi.id ORDER BY id DESC LIMIT 1) as return_type FROM `{$this->t('order_items')}` oi LEFT JOIN `{$this->t('users')}` u ON u.id=oi.seller_id LEFT JOIN `{$this->t('seller_profiles')}` vp ON vp.user_id=oi.seller_id WHERE oi.order_id=?", [$id]);
