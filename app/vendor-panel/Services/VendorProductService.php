...
class VendorProductService
{
    ...
    public function getAll(int $vendorId, int $page = 1, array $f = []): array {
        $w = "p.vendor_id=?"; $params = [$vendorId];
    ...
    public function findById(int $id, int $vendorId): ?array {
        $p = $this->db->fetchOne(
            "SELECT p.*, c.name as cat_name FROM `".DB_PREFIX."products` p LEFT JOIN `".DB_PREFIX."categories` c ON c.id=p.category_id WHERE p.id=? AND p.vendor_id=?",
            [$id, $vendorId]);
        if (!$p) return null;
    ...
    public function create(int $vendorId, array $d, array $files = []): array {
        ...
        $id = $this->db->insert(
            "INSERT INTO `".DB_PREFIX."products` (vendor_id,category_id,name,slug,description,price,sale_price,stock,sku,barcode,hsn_code,gst_rate,weight,is_featured,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            [$vendorId,$d['category_id'],$d['name'],$slug,$d['description']??null,(float)$d['price'],!empty($d['sale_price'])?(float)$d['sale_price']:null,(int)($d['stock']??0),$d['sku']??null,$d['barcode']??null,$d['hsn_code']??null,isset($d['gst_rate'])?(float)$d['gst_rate']:18.00,!empty($d['weight'])?(float)$d['weight']:null,(int)!empty($d['is_featured']),$d['status']??'active']
        );
    ...
    public function update(int $id, int $vendorId, array $d, array $files = []): array {
        $p = $this->findById($id, $vendorId);
        if (!$p) return ['success'=>false,'message'=>'Product not found.'];
        $this->db->execute(
            "UPDATE `".DB_PREFIX."products` SET category_id=?,name=?,description=?,price=?,sale_price=?,stock=?,sku=?,barcode=?,hsn_code=?,gst_rate=?,weight=?,is_featured=?,status=? WHERE id=? AND vendor_id=?",
            [$d['category_id'],$d['name'],$d['description']??null,(float)$d['price'],!empty($d['sale_price'])?(float)$d['sale_price']:null,(int)($d['stock']??0),$d['sku']??null,$d['barcode']??null,$d['hsn_code']??null,isset($d['gst_rate'])?(float)$d['gst_rate']:18.00,!empty($d['weight'])?(float)$d['weight']:null,(int)!empty($d['is_featured']),$d['status']??'active',$id,$vendorId]
        );
        $this->handleImages($id, $files);
        return ['success'=>true];
    }

    public function delete(int $id, int $vendorId): void {
        $this->db->execute("DELETE FROM `".DB_PREFIX."products` WHERE id=? AND vendor_id=?", [$id, $vendorId]);
    }

    public function toggle(int $id, int $vendorId): string {
        $p = $this->db->fetchOne("SELECT status FROM `".DB_PREFIX."products` WHERE id=? AND vendor_id=?", [$id, $vendorId]);
        if (!$p) return 'not_found';
        $new = $p['status'] === 'active' ? 'inactive' : 'active';
        $this->db->execute("UPDATE `".DB_PREFIX."products` SET status=? WHERE id=? AND vendor_id=?", [$new, $id, $vendorId]);
        return $new;
    }
