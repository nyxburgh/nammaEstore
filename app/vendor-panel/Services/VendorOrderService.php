...
class VendorOrderService
{
    ...
    {
        $where  = "oi.vendor_id = ?";
        $params = [$vendorId, $vendorId, $vendorId];   // JOIN, LEFT JOIN and WHERE each bind vendor_id
        ...
        $sql = "SELECT o.id, o.order_number, o.order_status, o.payment_status,
                       ...
                       ovs.vendor_earning, ovs.commission_pct
                FROM `".DB_PREFIX."orders` o
                JOIN `".DB_PREFIX."order_items` oi
                     ON oi.order_id = o.id AND oi.vendor_id = ?
                JOIN `".DB_PREFIX."users` u ON u.id = o.user_id
                LEFT JOIN `".DB_PREFIX."order_vendor_splits` ovs
                     ON ovs.order_id = o.id AND ovs.vendor_id = ?
                WHERE {$where}{$extra}
    ...
    public function findById(int $orderId, int $vendorId): ?array
    {
        $o = $this->db->fetchOne(
        ...
        $o['split'] = $this->db->fetchOne(
            "SELECT * FROM `".DB_PREFIX."order_vendor_splits`
             WHERE order_id = ? AND vendor_id = ?",
            [$orderId, $vendorId]
        );
        $o['shipment'] = $o['split'] ? $this->db->fetchOne(
            "SELECT * FROM `".DB_PREFIX."shipments` WHERE order_vendor_split_id = ?",
            [$o['split']['id']]
        ) : null;
        return $o;
    }

    public function updateStatus(int $orderId, int $vendorId, string $status, string $note = ''): array
    {
