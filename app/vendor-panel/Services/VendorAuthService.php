...
class VendorAuthService
{
    ...
    public function login(string $email, string $password): array {
        $rlKey = RateLimiter::keyFor('vendor_login', $email);
        ...
        // Auto-upgrade customer to vendor if no vendor profile yet
        $db = Database::getInstance();
        $vp = $db->fetchOne("SELECT * FROM `".DB_PREFIX."vendor_profiles` WHERE user_id=?", [$user['id']]);
        if (!$vp) return ['success' => false, 'message' => 'No vendor profile found. Please register as a vendor.'];
    ...
    public function register(array $d): array {
        ...
        try {
            ...
            // Self-registered vendors start pending/unverified — the
            // shop does NOT go live until the emailed OTP is confirmed.
            // Admin-created vendors (VendorService::create in the admin
            // panel) are unaffected by this and remain immediately active.
            $db->insert(
                "INSERT INTO `".DB_PREFIX."vendor_profiles` (user_id,shop_name,shop_slug,description,status) VALUES (?,?,?,?,'pending')",
            ...
            if ($plan) {
                $db->insert("INSERT INTO `".DB_PREFIX."vendor_subscriptions` (vendor_id,plan_id,status,started_at) VALUES (?,?,'active',NOW())", [$uid,$plan['id']]);
            }
            $db->commit();
            ...
            (new \App\Core\Services\OtpService())->send(
                $d['email'], 'vendor_email_verify',
                'Verify your email — ' . $d['shop_name'],
                'Your Namma E Store vendor verification code is'
            );
    ...
    public function verifyEmail(int $vendorId, string $email, string $code): array
    {
        $r = (new \App\Core\Services\OtpService())->verify($email, 'vendor_email_verify', $code);
        if (!$r['success']) return $r;

        $db = Database::getInstance();
        $db->execute(
            "UPDATE `".DB_PREFIX."vendor_profiles` SET status='active', approved_at=NOW() WHERE user_id=?",
            [$vendorId]
        );
        $db->execute(
    ...
    public function resendVerification(int $vendorId, string $email, string $shopName): array
    {
        return (new \App\Core\Services\OtpService())->send(
            $email, 'vendor_email_verify',
            'Verify your email — ' . $shopName,
            'Your Namma E Store vendor verification code is'
        );
    }

    public function logout(): void { Auth::logoutVendor(); }
}
