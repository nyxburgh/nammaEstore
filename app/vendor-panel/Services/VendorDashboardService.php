...
class VendorDashboardService
{
    ...
    public function getVendorProfile(int $vendorId): ?array {
        return $this->db->fetchOne(
            "SELECT vp.*, sp.name as plan_name
             FROM `".DB_PREFIX."vendor_profiles` vp
             LEFT JOIN `".DB_PREFIX."vendor_subscriptions` vs
                   ON vs.vendor_id = vp.user_id AND vs.status = 'active'
    ...
    public function getStats(int $vendorId): array {
        $revenue = (float)($this->db->fetchOne(
            "SELECT COALESCE(SUM(gross_amount),0) s FROM `".DB_PREFIX."order_vendor_splits` WHERE vendor_id=?", [$vendorId])['s'] ?? 0);
        $earning = (float)($this->db->fetchOne(
            "SELECT COALESCE(SUM(vendor_earning),0) s FROM `".DB_PREFIX."order_vendor_splits` WHERE vendor_id=?", [$vendorId])['s'] ?? 0);
        $orders  = (int)($this->db->fetchOne(
            "SELECT COUNT(DISTINCT order_id) c FROM `".DB_PREFIX."order_items` WHERE vendor_id=?", [$vendorId])['c'] ?? 0);
        $products= (int)($this->db->fetchOne(
            "SELECT COUNT(*) c FROM `".DB_PREFIX."products` WHERE vendor_id=? AND status='active'", [$vendorId])['c'] ?? 0);
        $rating  = (float)($this->db->fetchOne(
            "SELECT COALESCE(AVG(r.rating),0) avg FROM `".DB_PREFIX."reviews` r JOIN `".DB_PREFIX."products` p ON p.id=r.product_id WHERE p.vendor_id=? AND r.is_approved=1", [$vendorId])['avg'] ?? 0);
        $pending = (int)($this->db->fetchOne(
            "SELECT COUNT(DISTINCT oi.order_id) c FROM `".DB_PREFIX."order_items` oi JOIN `".DB_PREFIX."orders` o ON o.id=oi.order_id WHERE oi.vendor_id=? AND o.order_status IN ('placed','processing')", [$vendorId])['c'] ?? 0);
        return compact('revenue','earning','orders','products','rating','pending');
    }

    public function getRecentOrders(int $vendorId, int $limit = 10): array {
        return $this->db->fetchAll(
            "SELECT o.*, oi.product_name, oi.quantity, oi.unit_price, oi.subtotal as item_sub,
                    oi.id as item_id, u.name as customer_name,
                    ovs.commission_pct, ovs.commission_amount, ovs.vendor_earning
             FROM `".DB_PREFIX."orders` o
             JOIN `".DB_PREFIX."order_items` oi ON oi.order_id=o.id
             JOIN `".DB_PREFIX."users` u ON u.id=o.user_id
             LEFT JOIN `".DB_PREFIX."order_vendor_splits` ovs ON ovs.order_id=o.id AND ovs.vendor_id=?
             WHERE oi.vendor_id=?
             ORDER BY o.placed_at DESC LIMIT ?",
            [$vendorId, $vendorId, $limit]
        );
    }

    public function getTopProducts(int $vendorId, int $limit = 5): array {
        return $this->db->fetchAll(
    ...
    public function getSubscriptionInfo(int $vendorId): array {
        $commSvc  = new CommissionService();
        $plan     = $commSvc->getActivePlan($vendorId);
        $turnover = $commSvc->getMonthlyTurnover($vendorId);
    ...
    public function getWeeklyRevenue(int $vendorId): array {
        $rows = $this->db->fetchAll(
            "SELECT DATE(o.placed_at) as day, COALESCE(SUM(oi.subtotal),0) as revenue, COUNT(DISTINCT o.id) as orders
             FROM `".DB_PREFIX."orders` o
             JOIN `".DB_PREFIX."order_items` oi ON oi.order_id=o.id AND oi.vendor_id=?
    ...
    public function getEarningsSummary(int $vendorId, string $period = 'month'): array {
        $where = match($period) {
        ...
        $row = $this->db->fetchOne(
            "SELECT COALESCE(SUM(ovs.gross_amount),0) gross, COALESCE(SUM(ovs.commission_amount),0) comm, COALESCE(SUM(ovs.vendor_earning),0) net, COUNT(DISTINCT ovs.order_id) orders
             FROM `".DB_PREFIX."order_vendor_splits` ovs JOIN `".DB_PREFIX."orders` o ON o.id=ovs.order_id
             WHERE ovs.vendor_id=? AND $where", [$vendorId]
        );
        return $row ?? ['gross' => 0, 'comm' => 0, 'net' => 0, 'orders' => 0];
    }

    public function getCommissionLog(int $vendorId, int $page = 1): array {
        $sql = "SELECT c.*, o.order_number, p.name as product_name FROM `".DB_PREFIX."commissions` c
                ...
                WHERE c.vendor_id=? ORDER BY c.calculated_at DESC";
        return $this->db->paginate($sql, [$vendorId], $page);
    }

    public function getReviews(int $vendorId, int $page = 1): array {
        $sql = "SELECT r.*, p.name as product_name, u.name as user_name
                ...
                WHERE p.vendor_id=? ORDER BY r.created_at DESC";
        return $this->db->paginate($sql, [$vendorId], $page);
    }
}
