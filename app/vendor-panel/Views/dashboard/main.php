...
══════════════════════════════════════ -->
<div class="page-hd">
  <div><h1>📊 Overview</h1><p>Welcome back, <?= e($vendor['name']) ?>! Here's your store summary.</p></div>
  <div class="hd-actions">
    <a href="<?= VENDOR_URL ?>/products/add" class="btn-pink btn-sm">➕ Add Product</a>
  </div>
</div>
...
<div class="sub-banner">
  ...
  <?php endif; ?>
  <a href="<?= VENDOR_URL ?>/subscription" class="btn-outline btn-sm" style="background:rgba(255,255,255,.1);color:white;border-color:rgba(255,255,255,.3);">Upgrade Plan</a>
</div>
<?php endif; ?>
...
<div class="card">
  <div class="card-head">
    ...
    <a href="<?= VENDOR_URL ?>/orders" class="btn-outline btn-sm">View All</a>
  </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><a href="<?= VENDOR_URL ?>/orders/<?= $o['id'] ?>" class="btn-outline btn-sm">View</a></td>
          </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
...
<div class="card">
  <div class="card-head">
    ...
    <a href="<?= VENDOR_URL ?>/products" class="btn-outline btn-sm">Manage All</a>
  </div>
  <div class="card-body no-pad">
    <div class="table-wrap">
      <table class="data-table">
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="color:var(--green);font-weight:700;"><?= currency($o['vendor_earning']??$o['item_sub']) ?></td>
            <td><span class="status <?= e($o['order_status']) ?>"><?= ucfirst($o['order_status']) ?></span></td>
            <td style="display:flex;gap:4px;flex-wrap:wrap;">
              <a href="<?= VENDOR_URL ?>/orders/<?= $o['id'] ?>" class="btn-outline btn-sm">View</a>
...
<!-- ══════════════════════════════════════
     ORDER DETAIL
══════════════════════════════════════ -->
<div class="page-hd"><div><h1>📦 Order #<?= e($order['order_number']) ?></h1><p><?= formatDateTime($order['placed_at']) ?></p></div><div class="hd-actions"><a href="<?= VENDOR_URL ?>/orders" class="btn-outline btn-sm">← Back to Orders</a></div></div>
    ...
    <div class="card"><div class="card-head"><span class="card-title">🛍️ Order Items</span><span class="status <?= e($order['order_status']) ?>"><?= ucfirst($order['order_status']) ?></span></div>
      <div class="card-body no-pad">
        ...
        <div style="padding:14px;background:var(--green-pale);display:flex;justify-content:space-between;font-size:.82rem;">
          ...
          <span>Your Earning: <strong style="color:var(--green);"><?= currency($sp['vendor_earning']) ?></strong></span>
        </div>
        <?php endif; ?>
      </div>
    </div>
...
<!-- ══════════════════════════════════════
     PRODUCTS
══════════════════════════════════════ -->
<div class="page-hd"><div><h1>🛍️ My Products</h1><p>Manage your product catalogue</p></div><div class="hd-actions"><a href="<?= VENDOR_URL ?>/products/add" class="btn-pink btn-sm">➕ Add Product</a></div></div>
<form method="GET" class="filter-bar">
  <?= csrf_field() ?>
...
<div class="card">
  <div class="card-body no-pad">
    ...
    <div class="empty-state"><div class="es-icon">🛍️</div><h3>No products found</h3><p>Add your first product to start selling!</p><a href="<?= VENDOR_URL ?>/products/add" class="btn-pink">➕ Add Product</a></div>
    <?php else: ?>
    <div class="table-wrap">
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="display:flex;gap:4px;flex-wrap:wrap;">
              <a href="<?= VENDOR_URL ?>/products/<?= $p['id'] ?>/edit" class="btn-outline btn-sm">✏️ Edit</a>
              <button class="btn-outline btn-sm" onclick="toggleProductStatus(<?= $p['id'] ?>)">
                <?= $p['status']==='active'?'⏸️ Hide':'▶️ Show' ?>
              </button>
              <button class="btn-danger btn-sm" onclick="confirmDelete('<?= VENDOR_URL ?>/products/<?= $p['id'] ?>/delete','Delete this product?')">🗑️</button>
            </td>
          </tr>
          <?php endforeach; ?>
        </tbody>
...
<!-- ══════════════════════════════════════
     ADD / EDIT PRODUCT
══════════════════════════════════════ -->
<?php $isEdit = !empty($product); $formAction = $isEdit ? VENDOR_URL.'/products/'.$product['id'].'/edit' : VENDOR_URL.'/products/add'; ?>
<div class="page-hd">
  ...
  <div class="hd-actions"><a href="<?= VENDOR_URL ?>/products" class="btn-outline btn-sm">Cancel</a></div>
</div>
      ...
      <table class="data-table">
        ...
        <tbody id="commBody">
          ...
          <tr>
            ...
            <td style="color:var(--green);font-weight:700;"><?= currency($c['vendor_earning']) ?></td>
          </tr>
          <?php endforeach; endif; ?>
        </tbody>
      </table>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:start;">
  <div>
    <div class="card">
      ...
      <div class="card-body">
        <form method="POST" action="<?= VENDOR_URL ?>/settings">
  <?= csrf_field() ?>
          <div class="form-group" style="margin-bottom:12px;"><label class="form-label">Shop Name *</label><input class="form-input" type="text" name="shop_name" required value="<?= e($vendorProfile['shop_name']??'') ?>"></div>
          <div class="form-group" style="margin-bottom:12px;"><label class="form-label">Your Name</label><input class="form-input" type="text" name="name" value="<?= e($vendor['name']) ?>"></div>
          <div class="form-group" style="margin-bottom:12px;"><label class="form-label">Phone</label><input class="form-input" type="tel" name="phone" value="<?= e($vendor['phone']??'') ?>"></div>
          <div class="form-group" style="margin-bottom:12px;"><label class="form-label">Shop Phone</label><input class="form-input" type="tel" name="shop_phone" value="<?= e($vendorProfile['shop_phone']??'') ?>"></div>
          <div class="form-group" style="margin-bottom:12px;"><label class="form-label">Description</label><textarea class="form-input form-textarea" name="description"><?= e($vendorProfile['description']??'') ?></textarea></div>
  ...
  <div>
    <div class="card">
      ...
      <div class="card-body">
        <form method="POST" action="<?= VENDOR_URL ?>/settings">
  <?= csrf_field() ?>
          <div class="form-group" style="margin-bottom:10px;"><label class="form-label">Address</label><input class="form-input" type="text" name="address" value="<?= e($vendorProfile['address']??'') ?>" placeholder="Shop/Warehouse address"></div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
            <div class="form-group"><label class="form-label">City</label><input class="form-input" type="text" name="city" value="<?= e($vendorProfile['city']??'') ?>"></div>
            <div class="form-group"><label class="form-label">State</label><input class="form-input" type="text" name="state" value="<?= e($vendorProfile['state']??'') ?>"></div>
          </div>
          <div class="form-group" style="margin-bottom:10px;"><label class="form-label">Pincode</label><input class="form-input" type="text" name="pincode" value="<?= e($vendorProfile['pincode']??'') ?>" maxlength="6"></div>
          <div class="form-group" style="margin-bottom:12px;"><label class="form-label">GST Number</label><input class="form-input" type="text" name="gst_number" value="<?= e($vendorProfile['gst_number']??'') ?>" placeholder="22AAAAA0000A1Z5"></div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <form method="POST" action="<?= VENDOR_URL ?>/settings/password">
  <?= csrf_field() ?>
...
<div style="display:grid;grid-template-columns:1.3fr 1fr;gap:16px;">
  ...
  <div class="card">
    ...
    <div class="card-body">
      <form method="POST" action="<?= VENDOR_URL ?>/wallet/withdraw">
        <?= csrf_field() ?>
    ...
    <table class="data-table">
      ...
      <?php foreach(($invoicesData['data'] ?? []) as $inv): ?>
        <tr>
          ...
          <td><a href="<?= VENDOR_URL ?>/invoices/<?= $inv['id'] ?>/download" class="btn-pink" style="padding:5px 12px;font-size:.72rem;">⬇ Download</a></td>
        </tr>
      <?php endforeach; ?>
...
function loadEarnings(period, btn){
  ...
  fetch(VENDOR_URL+"/earnings?period="+period)
...
function updateStatus(orderId, status){
  ...
  fetch(VENDOR_URL+"/orders/"+orderId+"/status",{method:"POST",body:fd})
    .then(r=>r.json()).then(d=>{
...
function createShipment(e, orderId, splitId){
  ...
  fetch(VENDOR_URL+"/shipments/create",{method:"POST",body:fd})
    .then(r=>r.json()).then(d=>{
...
function updateShipmentStatus(shipmentId, status){
  ...
  fetch(VENDOR_URL+"/shipments/"+shipmentId+"/track",{method:"POST",body:fd})
    .then(r=>r.json()).then(d=>{
...
function markNotifRead(id, link){
  ...
  fetch(VENDOR_URL+"/notifications/"+id+"/read",{method:"POST",body:fd}).then(()=>{ if(link) location.href=link; });
}
