...
class OrderVendorSplitRepository extends Repository
{
    protected string $table = 'order_vendor_splits';
    ...
    public function getCandidatesForSettlement(): array
    {
        return $this->db->fetchAll(
            "SELECT ovs.*, o.delivered_at, o.order_status, o.payment_status, vp.vendor_type,
                    vp.user_id as vendor_user_id
             FROM `{$this->t()}` ovs
             JOIN `{$this->t('orders')}` o ON o.id=ovs.order_id
             JOIN `{$this->t('vendor_profiles')}` vp ON vp.user_id=ovs.vendor_id
    ...
    /** All order_item ids belonging to this vendor within this order — used to check for open returns/disputes. */
    public function getItemIds(int $orderId, int $vendorId): array
    {
        $rows = $this->db->fetchAll(
            "SELECT id FROM `{$this->t('order_items')}` WHERE order_id=? AND vendor_id=?",
            [$orderId, $vendorId]
        );
