...
<title><?= e($title??'Dashboard') ?> — Namma E Store Seller</title>
...
.seller-badge-top{background:rgba(255,255,255,.18);color:white;font-size:.68rem;font-weight:700;padding:3px 10px;border-radius:50px;letter-spacing:.5px;text-transform:uppercase;}
...
.seller-card-side{padding:20px 16px;border-bottom:1px solid rgba(255,255,255,.07);text-align:center;}
.seller-ava-big{width:60px;height:60px;border-radius:50%;margin:0 auto 10px;background:linear-gradient(135deg,var(--pink-main),var(--blue-main));border:2.5px solid var(--pink-main);display:flex;align-items:center;justify-content:center;font-size:1.8rem;font-weight:700;color:white;}
.seller-card-side h3{color:white;font-size:.9rem;font-weight:700;margin-bottom:3px;}
.seller-card-side p{color:rgba(255,255,255,.5);font-size:.7rem;margin-bottom:10px;}
...
<div class="top-bar">
  <div class="top-bar-left">
    ...
    <span class="seller-badge-top">🏪 Seller Panel</span>
  </div>
...
<div class="dash-layout">
  ...
  <aside class="sidebar" id="sidebar">
    <div class="seller-card-side">
      <div class="seller-ava-big"><?= strtoupper(substr($v['name'],0,1)) ?></div>
    ...
    <div class="nav-section">
      ...
      <a href="<?= SELLER_URL ?>/dashboard" class="nav-item <?= $sec==='overview'?'active':'' ?>"><span class="nav-icon">📊</span><span class="nav-label">Overview</span></a>
      <a href="<?= SELLER_URL ?>/orders" class="nav-item <?= $sec==='orders'||$sec==='order-detail'?'active':'' ?>"><span class="nav-icon">📦</span><span class="nav-label">Orders</span><?php if(is_array($stats) && !empty($stats['pending'])): ?><span class="nav-count"><?= $stats['pending'] ?></span><?php endif; ?></a>
      <a href="<?= SELLER_URL ?>/products" class="nav-item <?= $sec==='products'?'active':'' ?>"><span class="nav-icon">🛍️</span><span class="nav-label">Products</span></a>
      <a href="<?= SELLER_URL ?>/products/add" class="nav-item <?= $sec==='add-product'?'active':'' ?>"><span class="nav-icon">➕</span><span class="nav-label">Add Product</span></a>
    </div>
    <div class="sidebar-divider"></div>
    <div class="nav-section">
      <div class="nav-section-label">Finance</div>
      <a href="<?= SELLER_URL ?>/earnings?section=earnings" class="nav-item <?= $sec==='earnings'?'active':'' ?>"><span class="nav-icon">💰</span><span class="nav-label">Earnings</span></a>
      <a href="<?= SELLER_URL ?>/commission?section=commission" class="nav-item <?= $sec==='commission'?'active':'' ?>"><span class="nav-icon">📈</span><span class="nav-label">Commission</span></a>
      <a href="<?= SELLER_URL ?>/subscription?section=subscription" class="nav-item <?= $sec==='subscription'?'active':'' ?>"><span class="nav-icon">⭐</span><span class="nav-label">Subscription</span><span class="nav-count gold"><?= strlen($planName)>5?substr($planName,0,5).'…':$planName ?></span></a>
      <a href="<?= SELLER_URL ?>/wallet" class="nav-item <?= $sec==='wallet'?'active':'' ?>"><span class="nav-icon">👛</span><span class="nav-label">Wallet & Payouts</span></a>
      <a href="<?= SELLER_URL ?>/returns" class="nav-item <?= $sec==='returns'?'active':'' ?>"><span class="nav-icon">↩️</span><span class="nav-label">Returns</span></a>
      <a href="<?= SELLER_URL ?>/invoices" class="nav-item <?= $sec==='invoices'?'active':'' ?>"><span class="nav-icon">🧾</span><span class="nav-label">GST Invoices</span></a>
      <a href="<?= SELLER_URL ?>/notifications" class="nav-item <?= $sec==='notifications'?'active':'' ?>"><span class="nav-icon">🔔</span><span class="nav-label">Notifications</span></a>
    </div>
    <div class="sidebar-divider"></div>
    <div class="nav-section">
      <div class="nav-section-label">Manage</div>
      <a href="<?= SELLER_URL ?>/reviews?section=reviews" class="nav-item <?= $sec==='reviews'?'active':'' ?>"><span class="nav-icon">⭐</span><span class="nav-label">Reviews</span></a>
      <a href="<?= SELLER_URL ?>/settings" class="nav-item <?= $sec==='settings'?'active':'' ?>"><span class="nav-icon">⚙️</span><span class="nav-label">Settings</span></a>
    </div>
    <div class="sidebar-divider"></div>
    <div class="nav-section">
      <a href="<?= APP_URL ?>" class="nav-item"><span class="nav-icon">🏬</span><span class="nav-label">View My Store</span></a>
      <a href="<?= SELLER_URL ?>/logout" class="nav-item" style="color:rgba(255,100,100,.7);"><span class="nav-icon">🚪</span><span class="nav-label">Logout</span></a>
    </div>
  </aside>
...
<nav class="mob-bottom-nav">
  <div class="mob-bottom-inner">
    <a href="<?= SELLER_URL ?>/dashboard" class="mob-bottom-btn <?= $sec==='overview'?'active':'' ?>"><span class="bi">📊</span>Overview</a>
    <a href="<?= SELLER_URL ?>/orders" class="mob-bottom-btn <?= $sec==='orders'?'active':'' ?>"><span class="bi">📦</span>Orders</a>
    <a href="<?= SELLER_URL ?>/products/add" class="mob-bottom-btn <?= $sec==='add-product'?'active':'' ?>"><span class="bi">➕</span>Add</a>
    <a href="<?= SELLER_URL ?>/products" class="mob-bottom-btn <?= $sec==='products'?'active':'' ?>"><span class="bi">🛍️</span>Products</a>
    <a href="<?= SELLER_URL ?>/settings" class="mob-bottom-btn <?= $sec==='settings'?'active':'' ?>"><span class="bi">⚙️</span>Settings</a>
  </div>
</nav>
