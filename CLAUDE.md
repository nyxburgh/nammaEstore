...
Custom PHP MVC multi-seller marketplace, no framework. Three panels
...
| Seller dashboard | `/my-seller/` | `app/seller-panel/` |
...
define it in the panel's stylesheet under `assets/{admin,frontend,seller-panel}/css/`.
...
`assets/{admin,frontend,seller-panel}/js/` and wire it up with an event
...
└── seller-panel/          ← Controllers/Services/Views for /my-seller/
assets/{admin,frontend,seller-panel}/{css,js}/
```
